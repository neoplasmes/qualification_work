...
- Instead of `use-cases`, some kind of dumb Command-Query Responsibility Segregation is, used, where all write-related business logic, that will change some data is located in `write` folder (analog of commands), and all read-related one is in `read` folder (analog of queries). This modification is used for the purpose of better developer expirience, where software components definitions are closer to their mechanics rather than abstracted names, which are better to be used with UML diagrams or something.<br/>
**Naming convention:** `SignInHandler or SignUpHandler or LogoutHandler` for class name. Each handler **MUST** have an `execute` method. `signUp.write.ext or logout.write.ext` for files.
