...
describe('ActionsListPanel', () => {
    ...
    it('shows actions and opens a new draft', async () => {
        ...
        expect(screen.getByRole('button', { name: /Receive payment/ })).toBeInTheDocument();

        await user.click(screen.getByRole('button', { name: /New action/i }));

        expect(screen.getByRole('button', { name: /New action Draft.*Not saved/i })).toBeInTheDocument();
    });
