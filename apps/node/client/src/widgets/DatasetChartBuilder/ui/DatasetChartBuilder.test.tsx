...
describe('DatasetChartBuilder', () => {
    ...
    it('previews chart without saving, then saves on confirm', async () => {
        ...
        render(
            <MemoryRouter>
                <DatasetChartBuilder orgId="org-1" selectedDataset={dataset} onChartCreated={onChartCreated} />
            </MemoryRouter>
        );
        ...
        expect(previewChart).toHaveBeenCalledWith(expect.objectContaining({
            datasetId: 'dataset-1',
            chartType: 'heatmap',
    ...
    it('goes back to config form when Edit is clicked', async () => {
        ...
        await user.click(screen.getByRole('button', { name: /preview/i }));
        await waitFor(() => expect(screen.getByTestId('chart-result')).toBeInTheDocument());
