import { z } from 'zod';

import { parseWithZod } from '@qualification-work/microservice-utils/validation';
import { chartTypes } from '@qualification-work/types';

import type { UpdateChartCommand } from '@/core/commands';

import type { RequestHandlerType } from '@/shared/appState';

const patchChartSchema = z
    .object({
        name: z.string().min(1).max(255).optional(),
        chartType: z.enum(chartTypes).optional(),
        config: z.looseObject({}).optional(),
    })
    .refine(value => Object.keys(value).length > 0, {
        message: 'patch body is empty',
    });

export function createPatchChartHandler(handler: UpdateChartCommand): RequestHandlerType {
    return async ({ request, response }) => {
        const id = parseWithZod(z.uuid(), request.params.id);
        const rawBody = await request.json<unknown>();
        const body = parseWithZod(patchChartSchema, rawBody);

        await handler.execute(id, {
            name: body.name,
            chartType: body.chartType,
            config: body.config as never,
        });

        response.status(204).end();
    };
}
