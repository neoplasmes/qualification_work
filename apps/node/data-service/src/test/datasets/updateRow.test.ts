...
describe('PATCH /api/datasets/:id/rows/:rowId', () => {
    it('partially updates a row by jsonb merge', async () => {
        ...
        const res = await api(
            ...
            {
                method: 'PATCH',
                body: JSON.stringify({ values: { name: 'NEW' } }),
            }
        );
    ...
    it('rejects unknown column key', async () => {
        ...
        const res = await api(
            ...
            {
                method: 'PATCH',
                body: JSON.stringify({ values: { definitelyNotAColumn: 1 } }),
            }
        );
    ...
    it('rejects wrong type for column', async () => {
        ...
        const res = await api(
            ...
            {
                method: 'PATCH',
                body: JSON.stringify({ values: { age: 'not-a-number' } }),
            }
        );
    ...
    it('returns 403 for cross-org access', async () => {
        ...
        const res = await api(
            `/api/datasets/${datasetId}/rows/${row.id}?orgId=${outsiderOrg}`,
            {
                method: 'PATCH',
