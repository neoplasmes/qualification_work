...
const rule = {
    meta: {
        type: 'problem',
        docs: {
            description: 'use @/ alias for cross-slice imports, use relative paths for same-slice imports',
        },
        messages: {
        ...
        return {
            ImportDeclaration(node) {
                ...
                if (isRelative) {
                    ...
                    const importedLayerIndex = resolvedParts.findIndex(part => FSDLayers.includes(part));
                ...
                if (isAlias) {
                    ...
                    if (!FSDLayers.includes(importedLayer)) return;
